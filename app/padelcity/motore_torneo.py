"""
Motore del ciclo torneo di PadelCity - prima parte: generazione delle
...
from app.padelcity.models import Campionato, Torneo, IscrizioneTorneo, UtentePC, GruppoPC, GruppoMembroPC, ClassificaVoce
from app.padelcity.config import (
...
from app.padelcity.whatsapp_pc import (
...
from app.padelcity.formazione_gruppi import Candidato, forma_gruppi, assegna_lati
from app.padelcity.pdf_torneo import genera_pdf_torneo, _nome_campionato_leggibile
from app.padelcity.pdf_classifica import genera_pdf_classifica
from app.padelcity.email_pc import invia_pdf_torneo_segreteria
from app.padelcity.routing import imposta_contesto_attivo, rimuovi_contesto_attivo
...
def assicura_slot_campionati_esistano(db_pc: Session) -> int:
    """
...
def genera_tornei_futuri(db_pc: Session) -> int:
    """
    ...
    for offset in range(GIORNI_TORNEI_DA_GENERARE_IN_ANTICIPO):
        ...
        for campionato in campionati_di_quel_giorno:
            ...
            numero_tappa_attuale = db_pc.query(Torneo).filter(Torneo.campionato_id == campionato.id).count() + 1
            db_pc.add(Torneo(
...
def job_invia_richieste_iscrizione(db_pc: Session, torneo_id_specifico: int | None = None, ignora_soglia_tempo: bool = False) -> int:
    """
    ...
    query = db_pc.query(Torneo).filter(
...
def job_invia_solleciti_iscrizione(db_pc: Session, torneo_id_specifico: int | None = None, ignora_soglia_tempo: bool = False) -> int:
    """
    ...
    query = db_pc.query(Torneo).filter(Torneo.stato == "RICHIESTE_INVIATE", Torneo.attivo == True)  # noqa: E712
    if torneo_id_specifico is not None:
        query = query.filter(Torneo.id == torneo_id_specifico)
    tornei_da_elaborare = query.all()
...
def gestisci_risposta_bottone_iscrizione(db_pc: Session, iscrizione_id: int, confermato: bool) -> dict:
    """
...
def _mappa_compagni_precedenti(db_pc: Session, campionato_id: int, torneo_id_corrente: int) -> dict[int, set]:
    """
    ...
    torneo_precedente = (
        ...
        .join(GruppoPC, GruppoPC.torneo_id == Torneo.id)
    ...
    gruppi_precedenti = db_pc.query(GruppoPC).filter(GruppoPC.torneo_id == torneo_precedente.id).all()
    mappa: dict[int, set] = {}
    for gruppo in gruppi_precedenti:
        membri = db_pc.query(GruppoMembroPC).filter(GruppoMembroPC.gruppo_id == gruppo.id).all()
...
def job_forma_gruppi_torneo(db_pc: Session, torneo_id_specifico: int | None = None, ignora_soglia_tempo: bool = False) -> int:
    """
    ...
    query = db_pc.query(Torneo).filter(Torneo.stato == "SOLLECITO_INVIATO", Torneo.attivo == True)  # noqa: E712
    if torneo_id_specifico is not None:
        query = query.filter(Torneo.id == torneo_id_specifico)
    tornei_da_elaborare = query.all()

    for torneo in tornei_da_elaborare:
        campionato_corrente = db_pc.query(Campionato).filter(Campionato.id == torneo.campionato_id).first()
        if not ignora_soglia_tempo:
        ...
        if n_completi > 0:
            compagni_precedenti = _mappa_compagni_precedenti(db_pc, torneo.campionato_id, torneo.id)
            ...
            for indice, gruppo in enumerate(gruppi_formati, start=1):
                gruppo_db = GruppoPC(torneo_id=torneo.id, numero_gruppo=indice)
                db_pc.add(gruppo_db)
                db_pc.flush()

                assegnazione_lati = assegna_lati(gruppo)
                for candidato in gruppo:
                    db_pc.add(GruppoMembroPC(
        ...
        if n_completi > 0:
            pdf_bytes = genera_pdf_torneo(db_pc, torneo.id)
            if pdf_bytes is not None:
                nome_file = f"padelcity_gruppi_{torneo.data.isoformat()}.pdf"
                oggetto = f"PadelCity - Gruppi torneo {giorno_leggibile} {data_leggibile}"
                corpo_testo = (
...
def _prova_promuovi_prossima_riserva(db_pc: Session, torneo: Torneo, gruppo_id: int, lato_da_coprire: str) -> bool:
    """
    ...
    giorno_leggibile = _NOMI_GIORNI_LEGGIBILI[torneo.giorno_settimana].capitalize()
    data_leggibile = torneo.data.strftime("%d/%m")
    campionato_torneo = db_pc.query(Campionato).filter(Campionato.id == torneo.campionato_id).first()
    nome_campionato = _nome_campionato_leggibile(campionato_torneo)

    if candidata is None:
        gruppo = db_pc.query(GruppoPC).filter(GruppoPC.id == gruppo_id).first()
    ...
    utente = db_pc.query(UtentePC).filter(UtentePC.id == candidata.utente_id).first()
...
def richiedi_cancellazione_tardiva(db_pc: Session, iscrizione_id: int) -> dict:
    """
    ...
    membro = (
        db_pc.query(GruppoMembroPC)
        .filter(GruppoMembroPC.utente_id == iscrizione.utente_id)
        .join(GruppoPC, GruppoPC.id == GruppoMembroPC.gruppo_id)
        .filter(GruppoPC.torneo_id == torneo.id)
        .first()
    )
    if membro is None:
        return {"gestito": False, "motivo": "dati_incoerenti"}
    ...
    altri_membri = (
        db_pc.query(GruppoMembroPC)
        .filter(GruppoMembroPC.gruppo_id == gruppo_id, GruppoMembroPC.utente_id != iscrizione.utente_id)
        .all()
    )
    for altro in altri_membri:
        altro_utente = db_pc.query(UtentePC).filter(UtentePC.id == altro.utente_id).first()
    ...
    trovata_riserva = _prova_promuovi_prossima_riserva(db_pc, torneo, gruppo_id, lato_liberato)

    return {"gestito": True, "riserva_contattata": trovata_riserva}


def gestisci_risposta_promozione(db_pc: Session, iscrizione_id: int, accettata: bool) -> dict:
    """
    ...
    if scaduta:
        ...
        db_pc.commit()
        _prova_promuovi_prossima_riserva(db_pc, torneo, gruppo_id, lato_proposto)
    ...
    utente = db_pc.query(UtentePC).filter(UtentePC.id == iscrizione.utente_id).first()
    db_pc.add(GruppoMembroPC(gruppo_id=gruppo_id, utente_id=utente.id, lato_assegnato=lato_proposto))
    iscrizione.ruolo = "TITOLARE"
    db_pc.commit()
    ...
    membri_gruppo = db_pc.query(GruppoMembroPC).filter(GruppoMembroPC.gruppo_id == gruppo_id).all()
    nomi_compagni = []
    for membro in membri_gruppo:
        if membro.utente_id == utente.id:
            continue
        compagno = db_pc.query(UtentePC).filter(UtentePC.id == membro.utente_id).first()
...
def job_gestisci_promozioni_scadute(db_pc: Session) -> int:
    """
    ...
    for iscrizione in scadute:
        gestisci_risposta_promozione(db_pc, iscrizione.id, accettata=False)

    return len(scadute)


def job_richiedi_punteggio_torneo(db_pc: Session, torneo_id_specifico: int | None = None, ignora_soglia_tempo: bool = False) -> int:
    """
    ...
    query = db_pc.query(Torneo).filter(Torneo.stato.in_(["PDF_INVIATI", "GRUPPI_FORMATI"]), Torneo.attivo == True)  # noqa: E712
    if torneo_id_specifico is not None:
        query = query.filter(Torneo.id == torneo_id_specifico)
    tornei_da_elaborare = query.all()
    ...
    for torneo in tornei_da_elaborare:
        ...
        for gruppo in gruppi:
            ...
            for membro in membri:
                utente = db_pc.query(UtentePC).filter(UtentePC.id == membro.utente_id).first()
                if utente is None:
                    continue
                ...
                imposta_contesto_attivo(db_pc, utente.whatsapp_numero, "RICHIESTA_PUNTEGGIO", membro.id)
...
def job_sollecito_punteggio_torneo(db_pc: Session) -> int:
    ...
    da_sollecitare = (
        db_pc.query(GruppoMembroPC)
        .filter(
            GruppoMembroPC.stato_richiesta_punteggio == "IN_ATTESA",
            GruppoMembroPC.punteggio_richiesto_il.isnot(None),
            GruppoMembroPC.punteggio_richiesto_il < soglia_sollecito,
            GruppoMembroPC.punteggio_sollecito_inviato == False,  # noqa: E712
        )
        .all()
    )
    ...
    for membro in da_sollecitare:
        utente = db_pc.query(UtentePC).filter(UtentePC.id == membro.utente_id).first()
        gruppo = db_pc.query(GruppoPC).filter(GruppoPC.id == membro.gruppo_id).first()
        torneo = db_pc.query(Torneo).filter(Torneo.id == gruppo.torneo_id).first() if gruppo else None
...
def registra_punteggio_gruppo_membro(db_pc: Session, gruppo_membro_id: int, testo_ricevuto: str) -> dict:
    """
    ...
    """
    membro = db_pc.query(GruppoMembroPC).filter(GruppoMembroPC.id == gruppo_membro_id).first()
    if membro is None:
    ...
    if membro.stato_richiesta_punteggio == "RICEVUTO":
        rimuovi_contesto_attivo(db_pc, utente.whatsapp_numero)
...
def job_finalizza_punteggi_torneo(db_pc: Session) -> int:
    """
    ...
    for torneo in tornei_da_elaborare:
        ...
        membri = db_pc.query(GruppoMembroPC).filter(GruppoMembroPC.gruppo_id.in_(gruppo_ids)).all()
        if not membri:
            continue
        ...
        for membro in membri:
            if membro.punteggio_riportato is None:
                ...
                utente_senza_risposta = db_pc.query(UtentePC).filter(UtentePC.id == membro.utente_id).first()
                if utente_senza_risposta is not None:
                    rimuovi_contesto_attivo(db_pc, utente_senza_risposta.whatsapp_numero)
                continue
        ...
        pdf_classifica_bytes = genera_pdf_classifica(db_pc, torneo.campionato_id)
        if pdf_classifica_bytes is not None:
            nome_file_classifica = f"padelcity_classifica_{nome_campionato.replace(' ', '_')}_{torneo.data.isoformat()}.pdf"
            oggetto_classifica = f"PadelCity - Classifica aggiornata {nome_campionato} (dopo {giorno_leggibile} {data_leggibile})"
            corpo_classifica = (
...
def attiva_torneo(db_pc: Session, torneo_id: int) -> dict:
    """Riattiva un torneo precedentemente cancellato (finché non sono già stati formati i gruppi)."""
    torneo = db_pc.query(Torneo).filter(Torneo.id == torneo_id).first()
    if torneo is None:
...
def cancella_torneo(db_pc: Session, torneo_id: int) -> dict:
    """
    ...
    iscrizioni_gia_contattate = db_pc.query(IscrizioneTorneo).filter(IscrizioneTorneo.torneo_id == torneo.id).all()
