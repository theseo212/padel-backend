"""
Punto di instradamento tra il sistema generico AnnaPadel e PadelCity,
...
1. C'è un ButtonPayload con il prefisso di PadelCity? -> instradamento
   deterministico, gestito qui.
2. Non c'è nessun payload riconosciuto, ma il numero ha un "contesto
   attivo" PadelCity non scaduto (es. sta per rispondere con il
...
from app.padelcity.config import PREFISSO_ROUTING, MINUTI_VALIDITA_CONTESTO_ATTIVO
from app.padelcity.models import ContestoAttivoWhatsApp


def gestisci_webhook_padelcity(db_pc: Session, numero_mittente: str, dati: dict) -> bool:
    """
    ...
    Ritorna True se il messaggio era di competenza di PadelCity (gestito
    ...
    if button_payload.startswith(PREFISSO_ROUTING):
        ...
        _gestisci_risposta_bottone(db_pc, numero_mittente, azione)
        return True

    contesto = (
        db_pc.query(ContestoAttivoWhatsApp)
    ...
    if contesto and contesto.scade_il > datetime.utcnow():
        ...
        _gestisci_testo_libero(db_pc, numero_mittente, contesto, testo_messaggio)
        return True

    return False


def _gestisci_risposta_bottone(db_pc: Session, numero_mittente: str, azione: str) -> None:
    """
    ...
    /padelcity/rispondi/{token} in main.py e IscrizioneTorneo.codice_risposta.

    Questa funzione resta pronta per future interazioni PadelCity che
    useranno davvero bottoni Quick Reply a payload fisso (es. un singolo
    "Sì/No" senza bisogno di riferirsi a un id specifico).
    """
    print(f"[PADELCITY][BOTTONE] {numero_mittente} -> azione non riconosciuta: '{azione}'")


def _gestisci_testo_libero(db_pc: Session, numero_mittente: str, contesto: ContestoAttivoWhatsApp, testo: str) -> None:
    """
    ...
    if contesto.tipo_contesto == "RICHIESTA_PUNTEGGIO":
        from app.padelcity.motore_torneo import registra_punteggio_gruppo_membro
        registra_punteggio_gruppo_membro(db_pc, contesto.riferimento_id, testo)
        return

    print(
        f"[PADELCITY][TESTO LIBERO] {numero_mittente} -> "
        f"tipo='{contesto.tipo_contesto}' rif={contesto.riferimento_id} testo='{testo}' "
        f"(handler da implementare)"
    )


def imposta_contesto_attivo(db_pc: Session, numero_whatsapp: str, tipo_contesto: str, riferimento_id: int | None = None) -> None:
    """
    Da chiamare ogni volta che PadelCity manda un messaggio che si
    ...
    if esistente:
        ...
        db_pc.add(ContestoAttivoWhatsApp(
...
def rimuovi_contesto_attivo(db_pc: Session, numero_whatsapp: str) -> None:
    """Da chiamare quando la risposta attesa è arrivata ed è stata gestita."""
    db_pc.query(ContestoAttivoWhatsApp).filter(
