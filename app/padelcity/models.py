"""
Tabelle del sistema PadelCity — un solo circolo, tornei del mattino
...
from app.padelcity.database import BasePC

GIORNI_SETTIMANA = ["Lunedì", "Martedì", "Mercoledì", "Giovedì", "Venerdì", "Sabato", "Domenica"]


class UtentePC(BasePC):
    """
    Un giocatore iscritto ai tornei del mattino di PadelCity.
...
class Campionato(BasePC):
    """
    Un'edizione di uno dei NUMERO_CAMPIONATI campionati (slot) di
    PadelCity. Lo SLOT (1..NUMERO_CAMPIONATI) è l'identità stabile nel
    ...
    vincitore_utente_id = Column(Integer, ForeignKey("utenti_pc.id"), nullable=True)
...
class Torneo(BasePC):
    ...
    gruppi = relationship("GruppoPC", back_populates="torneo")


class IscrizioneTorneo(BasePC):
    """
    ...
    UtentePC.campionati_bitmask — questa è la conferma per la singola data).
    """
    __tablename__ = "iscrizioni_torneo"

    id = Column(Integer, primary_key=True)
    torneo_id = Column(Integer, ForeignKey("tornei.id"), nullable=False)
    utente_id = Column(Integer, ForeignKey("utenti_pc.id"), nullable=False)
    ...
    utente = relationship("UtentePC", back_populates="iscrizioni")
...
class GruppoPC(BasePC):
    """Un quartetto formato per un torneo specifico."""
    __tablename__ = "gruppi_pc"
    ...
    membri = relationship("GruppoMembroPC", back_populates="gruppo")


class GruppoMembroPC(BasePC):
    __tablename__ = "gruppi_membri_pc"

    id = Column(Integer, primary_key=True)
    gruppo_id = Column(Integer, ForeignKey("gruppi_pc.id", ondelete="CASCADE"), nullable=False)
    utente_id = Column(Integer, ForeignKey("utenti_pc.id"), nullable=False)
    ...
    gruppo = relationship("GruppoPC", back_populates="membri")
    utente = relationship("UtentePC")


class ClassificaVoce(BasePC):
    """
...
class ContestoAttivoWhatsApp(BasePC):
    """
    ...
    per numero di telefono, se PadelCity sta "aspettando" una risposta
