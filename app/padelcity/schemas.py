"""
Schemi dei dati scambiati tra il form web di PadelCity e il backend.
...
class IscrizionePCCreate(BaseModel):
    # dati anagrafici (usati solo se è la prima iscrizione in assoluto
    # su PadelCity - se il numero è già presente, questi campi vengono
...
class IscrizionePCResponse(BaseModel):
    utente_nuovo: bool
    richiede_validazione_otp: bool
    messaggio: str


class ValidaOtpPCRequest(BaseModel):
    whatsapp_numero: str
    codice_otp: str
