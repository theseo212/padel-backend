"""
Logica dell'iscrizione ai tornei del mattino di PadelCity: riconoscimento
...
from app.padelcity.models import UtentePC, Campionato
from app.padelcity.schemas import IscrizionePCCreate
from app.padelcity.whatsapp_pc import genera_otp, invia_otp_whatsapp, invia_riepilogo_iscrizione_pc
...
def campionati_bitmask_a_leggibile(db_pc: Session, bitmask: int) -> str:
    """
    ...
    nomi = []
    for slot in slot_selezionati:
        campionato = db_pc.query(Campionato).filter(Campionato.slot == slot, Campionato.stato == "APERTO").first()
...
def _numero_gia_verificato_nel_generico(db_generico: Session, whatsapp_numero: str) -> bool:
    ...
    Sola lettura: PadelCity non scrive mai nulla nel database generico.
    """
...
def profilo_pc(db_pc: Session, db_generico: Session, whatsapp_numero: str) -> dict:
    """
    Mirror di /utenti/profilo del sistema generico, per il riconoscimento
    nel form. Prima cerca nel database di PadelCity (match completo,
    ...
    PadelCity e vanno comunque scelti la prima volta qui.
    """
    utente = db_pc.query(UtentePC).filter(UtentePC.whatsapp_numero == whatsapp_numero).first()
...
def gestisci_iscrizione(db_pc: Session, db_generico: Session, dati: IscrizionePCCreate) -> dict:
    """
    Crea o aggiorna l'iscrizione di un utente PadelCity.
    ...
    """
    utente = db_pc.query(UtentePC).filter(UtentePC.whatsapp_numero == dati.whatsapp_numero).first()
    ...
    campionati_leggibili = campionati_bitmask_a_leggibile(db_pc, utente.campionati_bitmask)
    ...
    # in una precedente iscrizione a PadelCity): nessun OTP da rifare.
    riepilogo_inviato_davvero = invia_riepilogo_iscrizione_pc(
...
def valida_otp_pc(db_pc: Session, whatsapp_numero: str, codice_otp: str) -> dict:
    utente = db_pc.query(UtentePC).filter(UtentePC.whatsapp_numero == whatsapp_numero).first()
    if utente is None:
