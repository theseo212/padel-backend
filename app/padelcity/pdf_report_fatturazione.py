...
REGOLA DI CONTEGGIO: ogni GruppoPC esistente conta come una partita,
...
from app.padelcity.models import Torneo, Campionato, GruppoPC, GruppoMembroPC, UtentePC
...
def genera_pdf_report_fatturazione(db_pc: Session, anno: int, mese: int) -> bytes | None:
    """
    ...
    gruppi = (
        db_pc.query(GruppoPC)
        .join(Torneo, GruppoPC.torneo_id == Torneo.id)
        .filter(Torneo.data >= primo_giorno, Torneo.data <= ultimo_giorno)
        .order_by(Torneo.data, GruppoPC.numero_gruppo)
        .all()
    )
    if not gruppi:
        return None
    ...
    for gruppo in gruppi:
        torneo = db_pc.query(Torneo).filter(Torneo.id == gruppo.torneo_id).first()
