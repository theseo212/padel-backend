...
def genera_pdf_torneo(db_pc: Session, torneo_id: int) -> bytes | None:
    """
    ...
    gruppi = db_pc.query(GruppoPC).filter(GruppoPC.torneo_id == torneo.id).order_by(GruppoPC.numero_gruppo).all()
    if not gruppi:
        return None
    ...
    for indice, gruppo in enumerate(gruppi):
        ...
        membri = (
            ...
            .filter(GruppoMembroPC.gruppo_id == gruppo.id)
            .all()
        )
        cognomi = []
